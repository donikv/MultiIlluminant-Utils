...
function [white_R ,white_G ,white_B,output_data] = jvdw_general_cc(input_data,njet,mink_norm,sigma,mask_im)
...
tmp = max(max(max(input_data)));
if (tmp <= 255)
    saturation_threshold = 255;
else
    saturation_threshold = tmp+1; % some number. in this case, we probably have 16bit data, and don't want to clipp it
end
mask_im2 = mask_im + (jvdw_dilation33(double(max(input_data,[],3)>=saturation_threshold)));   
mask_im2=double(mask_im2==0);
mask_im2=jvdw_set_border(mask_im2,sigma+1,0);
...
if(njet==0)
   if(sigma~=0)
     for ii=1:3
        input_data(:,:,ii)=jvdw_gDer(input_data(:,:,ii),sigma,0,0);
     end
   end
end

if(njet>0)
    [Rx,Gx,Bx]=jvdw_norm_derivative(input_data, sigma, njet);
